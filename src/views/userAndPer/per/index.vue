<template>
  <div class="app-container">
    ...
    <el-table :data="sysMenuList" style="width: 100%;margin-bottom: 20px;margin-top: 10px;" row-key="id" border :default-expand-all="false" :tree-props="{children: 'children'}">
      <el-table-column prop="perName" label="菜单名称" width="160"/>
      ...
      <el-table-column prop="perType" label="权限类型" width="160">
        <template slot-scope="scope">
          {{ scope.row.perType === 1 ? '权限' : scope.row.perType === 2 ? '菜单权限' : '其他' }}
        </template>
      </el-table-column>
      <el-table-column prop="perPath" label="路由地址" width="120"/>
